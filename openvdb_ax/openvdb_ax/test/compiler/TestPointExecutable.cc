...
TestPointExecutable::testConstructionDestruction()
{
    ...
    laovdb::ax::PointExecutable::Ptr pointExecutable
        (new laovdb::ax::PointExecutable(C, E, emptyReg, nullptr, {}, tree));
...
TestPointExecutable::testCreateMissingAttributes()
{
    ...
    laovdb::points::PointDataGrid::Ptr
        grid = laovdb::points::createPointDataGrid
    ...
    laovdb::ax::PointExecutable::Ptr executable =
        compiler->compile<laovdb::ax::PointExecutable>("@a=v@b.x;");
    CPPUNIT_ASSERT(executable);

    executable->setCreateMissing(false);
    CPPUNIT_ASSERT_THROW(executable->execute(*grid), laovdb::AXExecutionError);
    ...
    CPPUNIT_ASSERT(bIdx != laovdb::points::AttributeSet::INVALID_POS);
    CPPUNIT_ASSERT(descriptor.valueType(bIdx) == laovdb::typeNameAsString<laovdb::Vec3f>());
    laovdb::points::AttributeHandle<laovdb::Vec3f>::Ptr
        bHandle = laovdb::points::AttributeHandle<laovdb::Vec3f>::create(leafIter->constAttributeArray(bIdx));
    CPPUNIT_ASSERT(bHandle->get(0) == laovdb::Vec3f::zero());

    const size_t aIdx = descriptor.find("a");
    CPPUNIT_ASSERT(aIdx != laovdb::points::AttributeSet::INVALID_POS);
    CPPUNIT_ASSERT(descriptor.valueType(aIdx) == laovdb::typeNameAsString<float>());
    laovdb::points::AttributeHandle<float>::Ptr
        aHandle = laovdb::points::AttributeHandle<float>::create(leafIter->constAttributeArray(aIdx));
...
TestPointExecutable::testGroupExecution()
{
    ...
    const std::vector<laovdb::Vec3d> positions = {
        {0,0,0},
        {1,1,1},
        {2,2,2},
        {3,3,3},
    ...
    auto checkValues = [&](const int expected)
    {
        ...
        for (; leafIter; ++leafIter) {
            laovdb::points::AttributeHandle<int> handle(leafIter->constAttributeArray(aIdx));
            CPPUNIT_ASSERT(handle.size() == 1);
            CPPUNIT_ASSERT_EQUAL(expected, handle.get(0));
        }
    };

    laovdb::points::appendAttribute<int>(grid->tree(), "a", 0);

    laovdb::ax::Compiler::UniquePtr compiler = laovdb::ax::Compiler::create();
    laovdb::ax::PointExecutable::Ptr executable =
        compiler->compile<laovdb::ax::PointExecutable>("i@a=1;");
    CPPUNIT_ASSERT(executable);
    ...
    laovdb::points::appendGroup(grid->tree(), group);

    // false group
    executable->execute(*grid);
    checkValues(0);

    laovdb::points::setGroup(grid->tree(), group, true);
...
TestPointExecutable::testCompilerCases()
{
    ...
    {
        // with string only
        CPPUNIT_ASSERT(static_cast<bool>(compiler->compile<laovdb::ax::PointExecutable>("int i;")));
        CPPUNIT_ASSERT_THROW(compiler->compile<laovdb::ax::PointExecutable>("i;"), laovdb::AXCompilerError);
        CPPUNIT_ASSERT_THROW(compiler->compile<laovdb::ax::PointExecutable>("i"), laovdb::AXSyntaxError);
        // with AST only
        auto ast = laovdb::ax::ast::parse("i;");
        CPPUNIT_ASSERT_THROW(compiler->compile<laovdb::ax::PointExecutable>(*ast), laovdb::AXCompilerError);
    }
    ...
    {
        laovdb::ax::PointExecutable::Ptr executable =
        compiler->compile<laovdb::ax::PointExecutable>("", logger); // empty
        CPPUNIT_ASSERT(executable);
    }
    logger.clear();
    {
        laovdb::ax::PointExecutable::Ptr executable =
            compiler->compile<laovdb::ax::PointExecutable>("i;", logger); // undeclared variable error
        CPPUNIT_ASSERT(!executable);
        CPPUNIT_ASSERT(logger.hasError());
        logger.clear();
        laovdb::ax::PointExecutable::Ptr executable2 =
            compiler->compile<laovdb::ax::PointExecutable>("i", logger); // expected ; error (parser)
    ...
    {
        laovdb::ax::PointExecutable::Ptr executable =
            compiler->compile<laovdb::ax::PointExecutable>("int i = 18446744073709551615;", logger); // warning
        CPPUNIT_ASSERT(executable);
    ...
    logger.clear();
    {
        ...
        CPPUNIT_ASSERT(tree);
        laovdb::ax::PointExecutable::Ptr executable =
            compiler->compile<laovdb::ax::PointExecutable>(*tree, logger); // empty
        CPPUNIT_ASSERT(executable);
    ...
    logger.clear();
    {
        ...
        CPPUNIT_ASSERT(tree);
        laovdb::ax::PointExecutable::Ptr executable =
            compiler->compile<laovdb::ax::PointExecutable>(*tree, logger); // undeclared variable error
        CPPUNIT_ASSERT(!executable);
        CPPUNIT_ASSERT(logger.hasError());
        logger.clear(); // no tree for line col numbers
        laovdb::ax::PointExecutable::Ptr executable2 =
    ...
    logger.clear();
    {
        ...
        CPPUNIT_ASSERT(tree);
        laovdb::ax::PointExecutable::Ptr executable =
            compiler->compile<laovdb::ax::PointExecutable>(*tree, logger); // warning
        CPPUNIT_ASSERT(executable);
        CPPUNIT_ASSERT(logger.hasWarning());
        logger.clear(); // no tree for line col numbers
        laovdb::ax::PointExecutable::Ptr executable2 =
    ...
    // with copied tree
    {
        ...
        laovdb::ax::PointExecutable::Ptr executable =
            compiler->compile<laovdb::ax::PointExecutable>(*copy, logger); // empty
        CPPUNIT_ASSERT(executable);
    }
    logger.clear();
    {
        ...
        std::unique_ptr<laovdb::ax::ast::Tree> copy(tree->copy());
        laovdb::ax::PointExecutable::Ptr executable =
            compiler->compile<laovdb::ax::PointExecutable>(*copy, logger); // undeclared variable error
        CPPUNIT_ASSERT(!executable);
    }
    logger.clear();
    {
        ...
        std::unique_ptr<laovdb::ax::ast::Tree> copy(tree->copy());
        laovdb::ax::PointExecutable::Ptr executable =
            compiler->compile<laovdb::ax::PointExecutable>(*copy, logger); // warning
        CPPUNIT_ASSERT(executable);
    }
    logger.clear();
}
...
TestPointExecutable::testExecuteBindings()
{
    ...
    // binding to different name existing attribute
    {
        laovdb::points::PointDataGrid::Ptr
            points = laovdb::points::createPointDataGrid
                <laovdb::points::NullCodec, laovdb::points::PointDataGrid>(singlePointZero, *defaultTransform);
        laovdb::points::appendAttribute<float>(points->tree(), "a");
        laovdb::ax::PointExecutable::Ptr executable =
            compiler->compile<laovdb::ax::PointExecutable>("@b = 1.0f;");
        CPPUNIT_ASSERT(executable);
        ...
        CPPUNIT_ASSERT(aidx != laovdb::points::AttributeSet::INVALID_POS);
        CPPUNIT_ASSERT(descriptor.valueType(aidx) == laovdb::typeNameAsString<float>());
        laovdb::points::AttributeHandle<float> handle(leafIter->constAttributeArray(aidx));
    ...
    // binding to existing attribute AND default bind other attribute
    {
        laovdb::points::PointDataGrid::Ptr
            points = laovdb::points::createPointDataGrid
                <laovdb::points::NullCodec, laovdb::points::PointDataGrid>(singlePointZero, *defaultTransform);
        laovdb::points::appendAttribute<float>(points->tree(), "a");
        laovdb::points::appendAttribute<float>(points->tree(), "c");
        laovdb::ax::PointExecutable::Ptr executable =
            compiler->compile<laovdb::ax::PointExecutable>("@b = 1.0f; @c = 2.0f;");
        CPPUNIT_ASSERT(executable);
        laovdb::ax::AttributeBindings bindings;
        ...
        CPPUNIT_ASSERT(cidx != laovdb::points::AttributeSet::INVALID_POS);
        CPPUNIT_ASSERT(descriptor.valueType(cidx) == laovdb::typeNameAsString<float>());
        laovdb::points::AttributeHandle<float> handle2(leafIter->constAttributeArray(cidx));
    ...
    // binding to non existent attribute, error
    {
        ...
        CPPUNIT_ASSERT_THROW(executable->execute(*points), laovdb::AXExecutionError);
    }
    ...
    // trying to bind to an attribute and use the original attribute name at same time
    {
        laovdb::points::PointDataGrid::Ptr
            points = laovdb::points::createPointDataGrid
                <laovdb::points::NullCodec, laovdb::points::PointDataGrid>(singlePointZero, *defaultTransform);
        laovdb::ax::PointExecutable::Ptr executable =
            compiler->compile<laovdb::ax::PointExecutable>("@b = 1.0f; @a = 2.0f;");
        CPPUNIT_ASSERT(executable);
        laovdb::ax::AttributeBindings bindings;
        bindings.set("b","a"); // bind b to a
    ...
    // bind P away from world space position to some other float attribute
    {
        laovdb::points::PointDataGrid::Ptr
            points = laovdb::points::createPointDataGrid
                <laovdb::points::NullCodec, laovdb::points::PointDataGrid>(singlePointZero, *defaultTransform);
        laovdb::ax::PointExecutable::Ptr executable =
            compiler->compile<laovdb::ax::PointExecutable>("f@P = 1.25f;");
        CPPUNIT_ASSERT(executable);
        laovdb::ax::AttributeBindings bindings;
    ...
    // bind P away from world space position to some other attribute, defaulting to vec3f (as P does)
    {
        laovdb::points::PointDataGrid::Ptr
            points = laovdb::points::createPointDataGrid
                <laovdb::points::NullCodec, laovdb::points::PointDataGrid>(singlePointZero, *defaultTransform);
        laovdb::ax::PointExecutable::Ptr executable =
            compiler->compile<laovdb::ax::PointExecutable>("@P = 1.25f;");
        CPPUNIT_ASSERT(executable);
        laovdb::ax::AttributeBindings bindings;
        bindings.set("P","a"); // bind float a to P
        ...
        const size_t aidx = descriptor.find("a");
        CPPUNIT_ASSERT(aidx != laovdb::points::AttributeSet::INVALID_POS);
        CPPUNIT_ASSERT(descriptor.valueType(aidx) == laovdb::typeNameAsString<laovdb::Vec3f>());
        laovdb::points::AttributeHandle<laovdb::Vec3f> handle(leafIter->constAttributeArray(aidx));
        CPPUNIT_ASSERT_EQUAL(laovdb::Vec3f(1.25f), handle.get(0));
    }

    // test setting bindings and then resetting some of those bindings on the same executable
    {
        laovdb::ax::PointExecutable::Ptr executable =
            compiler->compile<laovdb::ax::PointExecutable>("@b = 1.0f; @a = 2.0f; @c = 3.0f;");
        CPPUNIT_ASSERT(executable);
        laovdb::ax::AttributeBindings bindings;
        bindings.set("b","a"); // bind b to a
...
TestPointExecutable::testAttributeCodecs()
{
    ...
    // test supported truncated codecs
    {
        ...
        laovdb::ax::PointExecutable::Ptr executable =
            compiler->compile<laovdb::ax::PointExecutable>
