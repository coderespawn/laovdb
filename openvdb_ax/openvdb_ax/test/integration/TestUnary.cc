...
TestUnary::testUnaryVector()
{
    ...
    mHarness.addAttributes<laovdb::math::Vec3<int32_t>>
        (unittest_util::nameSequence("v3i", 4), {
            laovdb::math::Vec3<int32_t>(0, 1,-1),
            laovdb::math::Vec3<int32_t>(0,-1, 1),
            laovdb::math::Vec3<int32_t>(-1,-2,0),
            laovdb::math::Vec3<int32_t>(1, 0, 0)
        });

    mHarness.addAttributes<laovdb::math::Vec3<float>>
        (unittest_util::nameSequence("v3f", 2), {
            laovdb::math::Vec3<float>(0.0f, 1.1f,-1.1f),
            laovdb::math::Vec3<float>(0.0f,-1.1f, 1.1f),
        });

    mHarness.addAttributes<laovdb::math::Vec3<double>>
        (unittest_util::nameSequence("v3d", 2), {
            laovdb::math::Vec3<double>(0.0, 1.1,-1.1),
            laovdb::math::Vec3<double>(0.0,-1.1, 1.1),
        });

    // vec4

    mHarness.addAttributes<laovdb::math::Vec4<int32_t>>
        (unittest_util::nameSequence("v4i", 4), {
            laovdb::math::Vec4<int32_t>(0, 1,-1, 2),
            laovdb::math::Vec4<int32_t>(0,-1, 1, -2),
            laovdb::math::Vec4<int32_t>(-1,-2,0,-3),
            laovdb::math::Vec4<int32_t>(1, 0, 0, 0)
        });

    mHarness.addAttributes<laovdb::math::Vec4<float>>
        (unittest_util::nameSequence("v4f", 2), {
            laovdb::math::Vec4<float>(0.0f, 1.1f,-1.1f, 2.1f),
            laovdb::math::Vec4<float>(0.0f,-1.1f, 1.1f, -2.1f)
        });

    mHarness.addAttributes<laovdb::math::Vec4<double>>
        (unittest_util::nameSequence("v4d", 2), {
            laovdb::math::Vec4<double>(0.0, 1.1,-1.1, 2.1),
            laovdb::math::Vec4<double>(0.0,-1.1, 1.1, -2.1)
        });
