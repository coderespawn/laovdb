...
{
    std::vector<laovdb::Vec3d> positions =
    ...
    laovdb::points::PointDataTree* tree = &(grid->tree());

    // @note  snippet moves all points to a single leaf node
    CPPUNIT_ASSERT_EQUAL(laovdb::points::pointCount(*tree), laovdb::Index64(4));

    const std::string code = unittest_util::loadText("test/snippets/worldspace/worldSpaceAssign");
    CPPUNIT_ASSERT_NO_THROW(laovdb::ax::run(code.c_str(), *grid));
    ...
    const laovdb::math::Transform& transform = grid->transform();
    for (; leaf; ++leaf)
    {
        CPPUNIT_ASSERT(leaf->pointCount() == 4);

        AttributeHandle<laovdb::Vec3f>::Ptr pOriginalHandle = AttributeHandle<laovdb::Vec3f>::create(leaf->attributeArray("P_original"));
        AttributeHandle<laovdb::Vec3f>::Ptr pNewHandle = AttributeHandle<laovdb::Vec3f>::create(leaf->attributeArray("P_new"));
        AttributeHandle<laovdb::Vec3f>::Ptr pHandle = AttributeHandle<laovdb::Vec3f>::create(leaf->attributeArray("P"));

        for (auto voxel = leaf->cbeginValueAll(); voxel; ++voxel) {
            const laovdb::Coord& coord = voxel.getCoord();
            auto iter = leaf->beginIndexVoxel(coord);
            for (; iter; ++iter) {

                const laovdb::Index idx = *iter;

                // test that the value for P_original
                const laovdb::Vec3f& oldPosition = positions[idx];
                const laovdb::Vec3f& pOriginal = pOriginalHandle->get(idx);
                ...
                const laovdb::Vec3f newPosition = laovdb::Vec3f(2.22f, 3.33f, 4.44f);
                const laovdb::Vec3f& pNew = pNewHandle->get(idx);
                ...
                const laovdb::Vec3f voxelSpacePosition = laovdb::Vec3f(0.2f, 0.3f, 0.4f);
                const laovdb::Vec3f& pVoxelSpace = pHandle->get(idx);
                ...
                const laovdb::Vec3f positionWS = laovdb::Vec3f(2.22f, 3.33f, 4.44f);
                const laovdb::Vec3f pWS = transform.indexToWorld(coord.asVec3d() + pHandle->get(idx));
...
TestWorldSpaceAccessors::testWorldSpaceAssignComponent()
{
    ...
    const laovdb::Index64 originalCount = pointCount(tree);
...
TestWorldSpaceAccessors::testWorldSpaceAssignBound()
{
    ...
    CPPUNIT_ASSERT_NO_THROW(laovdb::ax::run(code.c_str(), *grid, {{"pos","P"}}));
